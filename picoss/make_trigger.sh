...
# data and network configuration parameters. THis file can be further loaded with the


data_main="data" #the folder where we have all the data stored.
...
#we call the method
python picos_gui/utils/trigerring.py -o $DATA_folder -d $DEST_folder -l $trig_on $trig_off $nsta $nlta $trigger_type $up_freq $low_freq
